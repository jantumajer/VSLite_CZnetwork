...
leakybucket.monthly <- function(syear,eyear,phi,T,P,Mmax = 0.76,Mmin = 0.01,alph = 0.093,
  ...
  Tm <- rowMeans(T);
  # Tm <- rowMeans(T[,c(1:60)]) # Hack for simulation of potEV
  istar <- (Tm/5)^1.514; 
  istar[Tm < 0] <- 0;
  I <- sum(istar);
  ...
  for (cyear in 1:nyrs){     # begin cycling over years
    ...
    for (t in 1:12){  # begin cycling over months in a year
      ...
      ##### Compute potential evapotranspiration for current month after Thornthwaite (1948) and Willlmott et al. (1985):
      if ( T[t,cyear] < 0 ){Ep = 0;}
