...
# SITE <- read_xlsx("E:/TACR/VS_for_GACR/GACR_Model_Revision/metadata_GACR_SK_REMOTEF.xlsx")
...
for (i in c(1093:nrow(SITE))){
  ...
  if(i < 10 | i > 1092){series <- read.rwl(paste("E:/TACR/VS_for_GACR/GACR_Model_Revision/RWL_sites/", rwl.code, sep = ""))
            }else{series <- read.tridas(paste("E:/TACR/VS_for_GACR/GACR_Model_Revision/RWL_sites/", rwl.code, sep = ""))$measurements}
  ...
  if (nrow(chronology.sub) > 40 & "1961" %in% row.names(chronology.sub) & "1995" %in% row.names(chronology.sub)){
  syear <- 1961
  ...
  if(max(tuning$best$correlation) > -1){
    ...
    cor.save[i] <- simulation$par$corr
    print(mean(cor.save, na.rm = T))
    }
  }
}
